...
// [[Rcpp::export]]
arma::mat simrRUM(unsigned int N,unsigned int J, unsigned int K,const arma::mat& Q,
...
// [[Rcpp::export]]
arma::mat simgnida(unsigned int N,unsigned int J, unsigned int K,const arma::mat& Q,
                   const arma::mat& B0,const arma::mat& B1,
                   const arma::vec& CLASS){
...
// [[Rcpp::export]]
void parm_update_nomiss(unsigned int N,unsigned int J,
                        unsigned int K,unsigned int nClass,
                        const arma::mat& Y,
                        arma::mat& X_biject,
                        arma::mat& B0,
                        arma::mat& B1,
    ...
    for(unsigned int i=0;i<N;i++){
        ...
        for(unsigned int k=0;k<K;k++){
            ...
            u = R::runif(0.,1.);
            double lnA = arma::accu((Zi.col(k))%B1.col(k))-B0k_B1k(k)-B1k_B1k_dividedby2(k)+log(pis(c_aik_1));
    ...
    for(unsigned int j=0;j<J;j++){
        ...
        for(unsigned int k=0;k<K;k++){
            ...
            double lnA=.5*log(v1)-.5*v1*B1jk*B1jk+.5*log(v0)-.5*v0*B0jk*B0jk+log(pdjk);
            double lnB=.5*log(cv1)-.5*cv1*B1jk*B1jk+.5*log(cv0)-.5*cv0*B0jk*B0jk+log(1.-pdjk);
...
// [[Rcpp::export]]
Rcpp::List update_Gamma_Delta_MVN(unsigned int N,
                                  unsigned int V,
                                  unsigned int K,unsigned int J,const arma::mat& Y,
                                  arma::mat& Gamma,arma::mat& deltas,const arma::mat& X,
                                  const arma::vec& diagXpX,double w2,double nu,double bnu){
    ...
    if(V>1){
        for(unsigned int vv=1;vv<V;++vv){
            ...
            arma::vec lnb_m_lna=log(1.-nu)-.5*pow(mu_v,2)/sigv2-log(nu)-.5*(log(w2)+log(sigv2));
            for(unsigned int k=0;k<K;++k){
                arma::uvec notk = find(onetoK != k);
                double uvk = R::runif(0,1);
                double lnuvk = log(uvk);
                double ln1muvk = log(1.-uvk);
                //double dvk = (1.-(arma::accu(delta_v(notk))>0.) )*(ln1muvk-lnuvk>lnb_m_lna(k));//imposes structured sparsity
    ...
    double w2new = R::qgamma(uw,aforw2/2.0+1.,1.0/(.5*bforw2+1.),1,0);
    //update nu
    double u_nu=R::runif(0,1);
    double nu_new = R::qbeta(u_nu,aforw2+1.,(V-1.)*K-aforw2+bnu,1,0);
...
Rcpp::List rRUM_mvnQ_Gibbs(const arma::mat& Y,unsigned int K,
    ...
    arma::mat deltas = arma::zeros<arma::mat>(V,K);//arma::randi<arma::vec>(V,arma::distr_param(0,K));
    ...
    return Rcpp::List::create(Rcpp::Named("PISTAR",PISTAR),
                              ...
                              Rcpp::Named("PIs",PIs),
                              Rcpp::Named("QS",QS),
                              Rcpp::Named("GAMMAS",GAMMAS_OUT),
                              Rcpp::Named("DELTAS",Delta_tab)
                                  //Rcpp::Named("B0s",B0s),
                                  //Rcpp::Named("B1s",B1s),
